...
class Predictor(nn.Module):
    def __init__(self, ResMLP, Affine, dim, window, window2, layer_gnn, layer_cnn, layer_output):
        ...
        self.WC_struc2 = ResMLP(Affine, 64, 75, 75, 0,False, True) 
        ...
        self.WC_finger1 = ResMLP(Affine, 1, 600, 75, 0)
        self.WC_finger2 = ResMLP(Affine, 1, 600, 75, 0,False, True)
    ...
    def forward(self, inputs):
        ...
        micro_vectors = self.dropout(self.attention_PM(micro_vector, finger_vector))  # .permute(0, 2, 1)
        finger_vectors = self.dropout(self.attention_PM2(finger_vector, micro_vector))
...
def train(max_epochs, model, optimizer, scheduler, dataset_train, dataset_test):
    best_val_loss = 100
    ...
    for epoch in range(max_epochs):
        ...
        val_loss, mae_loss,r2, outputs,labels = get_metrics(model, dataset_test)
...
def main(max_epochs,dataset_train,dataset_test,dropout):
    model = Predictor(ResMLP, Affine, dim, window, window2, layer_gnn, layer_cnn, layer_output).to(device)
